...
- 箱子命名通过箱子物品菜单中的“改名”按钮触发，适用于所有 `playerChest`，包括普通箱、大型箱、冰箱和特殊玩家箱；关闭箱子且没有活动菜单时，自定义名称贴在箱子本体上部显示，被玩家占用或正在打开菜单时不显示标签。使用原版 `NamingMenu` 输入与脏词过滤，最多 32 个字符。空白提交恢复默认名称“Chest”；自定义名称与原始默认名称随箱子存档并通过原生网络字段同步，替换箱子类型时也保留。非玩家战利品或礼物箱不属于可命名范围。
...
- 矿井梯子定位按当前矿层累计被移除且名称为 `Stone` 的普通石头；加入者通过原版同步的 `stonesLeftOnThisLevel` 观察主机造成的破坏，不依赖加入者本地的 `ObjectListChanged`。普通楼梯和骷髅洞穴的洞都视为已经出现下降点，并停止提示和兜底；若前 10 块石头后仍无下降点，主机在第 11 块石头的位置安排必出的下降点。第 11 块的兜底延后一帧，先让原版下降点落地，避免把同格的洞覆盖为楼梯；没有下一层的矿层不执行该保证。
