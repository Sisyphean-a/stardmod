...
- 矿井梯子定位按当前矿层累计被移除且名称为 `Stone` 的普通石头；加入者通过原版同步的 `stonesLeftOnThisLevel` 观察主机造成的破坏，不依赖加入者本地的 `ObjectListChanged`。若前 10 块石头后仍未生成楼梯，主机在第 11 块石头的位置安排必出的楼梯；没有下一层的矿层不执行该保证。
